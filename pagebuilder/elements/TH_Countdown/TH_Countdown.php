...
class TH_Countdown extends ZnElements
{
	...
	public function css(){
		...
		// Digit Styles
		if( $this->opt('typo', $text_color_std ) ){
			$css .= zn_typography_css(array(
					'selector' => '.'.$uid.' .kl-counter-li',
					'lg' =>  $this->opt('typo', '' ),
				)
			);
		}
		...
		// Unit Styles
		if( $this->opt('unit_typo', '' ) ){
			$css .= zn_typography_css(array(
					'selector' => '.'.$uid.' .kl-counter-unit',
					'lg' =>  $this->opt('unit_typo', '' ),
				)
			);
		}
...
var th_counterOptions = {
	layout: function ()
	{
		return '<li class="kl-counter-li">{dn}<span class="kl-counter-unit {$shown}">{dl}</span></li>' +
			'<li class="kl-counter-li">{hn}<span class="kl-counter-unit {$shown}">{hl}</span></li>' +
			'<li class="kl-counter-li">{mn}<span class="kl-counter-unit {$shown}">{ml}</span></li>' +
			'<li class="kl-counter-li">{sn}<span class="kl-counter-unit {$shown}">{sl}</span></li>';
	}
};
