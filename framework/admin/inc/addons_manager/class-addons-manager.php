...
class ZnAddonsManager{
	...
	function __construct(){
		...
		if( file_exists( THEME_BASE .'/template_helpers/plugins/config.php' ) ) {
            ...
            $plugins = include THEME_BASE .'/template_helpers/plugins/config.php';
			if( is_array( $plugins ) ){
	...
	public function zn_do_plugin_action( $isAjaxCall = true, array $options = array() )
	{
		...
		switch ( $action ) {
			...
			case 'install_plugin':
				$credentials = false;
				if(isset($_POST) && isset($_POST['credentials'])){
	...
	function _do_plugin_install( $slug, $saved_credentials = false){
		$result = $this->do_plugin_install( $slug, $saved_credentials );
	 ...
	 * @return array|bool
	 */
	function do_plugin_install( $slug, $saved_credentials = false, $pluginSource = '' ) {
	...
	public function get_download_url( $slug, $pluginSource = '' ) {
		...
		switch ( $pluginSource ) {
			...
			default: {
				if (false !== filter_var($pluginSource, FILTER_VALIDATE_URL)) {
					return $pluginSource;
				}
			}
		}
